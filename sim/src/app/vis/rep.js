// https://github.com/cagataydemiralp/rep.git Version 0.1.0. Copyright 2017 Cagatay Demiralp.
(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports) :
	typeof define === 'function' && define.amd ? define(['exports'], factory) :
	(factory((global.rep = global.rep || {})));
...
var stat = createCommonjsModule(function (module) {
!function() {
  var stat = {
    version: "0.1.0"
  };
  stat.array_ = {};
  stat.array_.new = function(s, v) {
    return arguments.length === 1 ? new Array(s) : stat.array_.init(new Array(s), v);
  };
  stat.array_.init = function(a, v) {
    var n = a.length, i;
    if (typeof v === "function") for (i = 0; i < n; a[i] = v(i++)) ; else for (i = 0; i < n; a[i++] = v) ;
    return a;
  };
  stat.array_.copy2D = function(a) {
    var n = a.length, b = [], i;
    for (i = 0; i < n; b.push(a[i++].slice())) ;
    return b;
  };
  stat.array_.toNum = function(v) {
    var n = v.length, i;
    for (i = 0; i < n; v[i] = +v[i++]) ;
    return v;
  };
  stat.array_.range = function(n) {
    var a = stat.array_.new(n), i;
    for (i = 0; i < n; a[i] = i++) ;
    return a;
  };
  stat.array_.sort = function(a) {
    var n = a.length, ca = function(x, y) {
      return x - y;
    }, ci = function(x, y) {
      return a[x] - a[y];
    }, indices = stat.array_.range(n).sort(ci);
    return {
      array: a.sort(ca),
      indices: indices
    };
  };
  stat.array_.sortDescending = function(a) {
    var n = a.length, ca = function(x, y) {
      return y - x;
    }, ci = function(x, y) {
      return a[y] - a[x];
  ...
  stat.random = {};
  stat.random.randi = function(low, high, size) {
    var i, s;
    if (!arguments.length) {
      s = Math.floor(Math.random() * 2);
    } else if (arguments.length === 1) {
      s = Math.floor(Math.random() * low);
    } else if (arguments.length === 2) {
      s = Math.floor(Math.random() * (high - low)) + low;
    } else {
      s = stat.array(size);
      for (i = 0; i < size; i++) s[i] = Math.floor(Math.random() * (high - low)) + low;
    }
    return s;
  };
  stat.random.rand = function(low, high, size) {
    var i, s;
    if (!arguments.length) {
      s = Math.random();
    } else if (arguments.length === 1) {
      ...
      s = Math.random() * (high - low) + low;
    } else {
      s = stat.array(size);
      for (i = 0; i < size; i++) s[i] = Math.random() * (high - low) + low;
    }
    return s;
  };
  stat.random.randn = function(mean, stdev, size) {
    var s;
    mean = mean || 0;
    stdev = stdev || 1;
    if (arguments.length < 3) {
      s = stat.random.normal(mean, stdev);
    } else {
      s = stat.array(size);
      for (var i = 0; i < size; i++) s[i] = stat.random.normal(mean, stdev);
    }
    return s;
  };
  stat.random.permute = function(a) {
    var n = a.length, v, j;
    for (var i = --n; i >= 1; i--) {
      j = ~~(Math.random() * (i + 1));
      v = a[i];
  ...
  stat.random.comb = function(a, k) {
    var c = [], C = [];
    stat_random_subcomb(0, k, a, c, C);
    return C;
  };
  function stat_random_subcomb(i, k, a, c, C) {
    var n = a.length;
    if (k == 0) {
      C.push(c.slice());
      return;
    ...
    for (var j = i; j <= n - k; j++) {
      c.push(a[j]);
      stat_random_subcomb(j + 1, k - 1, a, c, C);
      c.pop();
    }
  }
  stat.random.randnTrunc = function() {};
  stat.random.normal = function(mean, stdev) {
    var x = 0, y = 0, s, rds, c;
    mean = mean || 0;
    stdev = stdev || 1;
    if (stat.random.nextg) {
      s = stat.random.nextg;
      stat.random.nextg = undefined;
    } else {
      do {
        x = Math.random() * 2 - 1;
        y = Math.random() * 2 - 1;
      ...
      stat.random.nextg = mean + y * c * stdev;
      s = mean + x * c * stdev;
    }
    return s;
  };
  ...
  stat.vector = {};
  stat.vector.new = function(n, val) {
    return arguments.length === 1 ? new Array(n) : stat.vector.init(new Array(n), val);
  };
  stat.vector.init = function(v, val) {
    var n = v.length, i;
    if (typeof val === "function") for (i = 0; i < n; v[i] = val(i++)) ; else for (i = 0; i < n; v[i++] = val) ;
    return v;
  };
  stat.vector.toNum = function(v) {
    return stat.array_.toNum(v);
  };
  ...
  stat.vector.subtractScalar = function(v0, s) {
    var v = [], n = v0.length, i;
    for (i = 0; i < n; i++) v.push(v0[i] - s);
    return v;
  };
  ...
  stat.vector.scalar = function(s, v) {
    var u = [], n = v.length, i;
    for (i = 0; i < n; i++) u.push(s * v[i]);
    return u;
  };
  stat.vector.setDiff = function(v0, v1) {
    return v0.filter(function(x) {
      return v1.indexOf(x) < 0;
    });
  };
  stat.vector.setUnion = function(v0, v1) {};
  stat.vector.setIntersect = function(v0, v1) {};
  ...
  stat.vector.euclidean = function(v0, v1) {
    return stat.vector.norm(stat.vector.subtract(v0, v1));
  };
  stat.vector.round = function(v0) {
    var n = v0.length, v = [], i;
    for (i = 0; i < n; v.push(Math.round(v0[i++]))) ;
    return v;
  };
  stat.vector.abs = function(v0) {
    var n = v0.length, v = [], i;
    for (i = 0; i < n; v.push(Math.abs(v0[i++]))) ;
    return v;
  };
  stat.vector.max = function(v) {
    var max = v[0], n = v.length, f = n % 2, i, val;
    for (i = 1; i < n; i += 2) {
      val = v[i] > v[i + 1] ? v[i] : v[i + 1];
      max = max < val ? val : max;
    }
    if (!f) {
      val = v[n - 1];
  ...
  stat.matrix.assignScalar = function(m, s) {
    var numRows = m.length, numCols = m[0].length, i, j;
    for (i = 0; i < numRows; i++) for (j = 0; j < numCols; j++) m[i][j] = s;
    return m;
  };
  stat.matrix.copy = function(m) {
    var numRows = m.length, mcopy = [], i;
    for (i = 0; i < numRows; mcopy.push(m[i++].slice())) ;
    return mcopy;
  };
  stat.matrix.sum = function(m, dim, v) {
    var numRows = m.length, numCols = m[0].length, i, j, s;
    dim = dim || 0;
    if (dim === 0) {
      s = v || stat.vector.new(numCols, 0);
      for (i = 0; i < numCols; i++) for (j = 0; j < numRows; j++) s[i] += m[j][i];
    } else if (dim === 1) {
      s = v || stat.vector.new(numRows, 0);
      for (i = 0; i < numRows; i++) for (j = 0; j < numCols; j++) s[i] += m[i][j];
    }
    return s;
  };
  stat.matrix.weightedSum = function(m, w, dim, v) {
    var numRows = m.length, numCols = m[0].length, i, j, s;
    dim = dim || 0;
    if (dim === 0) {
      s = v || stat.vector.new(numCols, 0);
      for (i = 0; i < numCols; i++) for (j = 0; j < numRows; j++) s[i] += w[j] * m[j][i];
    } else if (dim === 1) {
      s = v || stat.vector.new(numRows, 0);
      for (i = 0; i < numRows; i++) for (j = 0; j < numCols; j++) s[i] += w[i] * m[i][j];
    }
    return s;
  };
  stat.matrix.median = function(m, dim, v) {};
  stat.matrix.max = function(m, dim, v) {
    var numRows = m.length, numCols = m[0].length, i, j, max, f, max0;
    dim = dim || 0;
    if (dim === 0) {
      max = v || stat.vector.new(numCols);
      f = numRows % 2;
      for (i = 0; i < numCols; i++) {
        max[i] = m[0][i];
        for (j = 0; j < numRows - f; j += 2) {
          max0 = m[j][i] < m[j + 1][i] ? m[j + 1][i] : m[j][i];
          max[i] = max[i] < max0 ? max0 : max[i];
        }
        if (f) {
          max0 = m[numRows - 1][i];
          max[i] = max[i] < max0 ? max0 : max[i];
      ...
      max = v || stat.vector.new(numRows);
      f = numCols % 2;
      for (i = 0; i < numRows; i++) {
        max[i] = m[i][0];
        for (j = 0; j < numCols - f; j += 2) {
          max0 = m[i][j] < m[i][j + 1] ? m[i][j + 1] : m[i][j];
          max[i] = max[i] < max0 ? max0 : max[i];
        }
        if (f) {
          max0 = m[i][numCols - 1];
  ...
  stat.matrix.min = function(m, dim, v) {
    var numRows = m.length, numCols = m[0].length, i, j, min, f, min0;
    dim = dim || 0;
    if (dim === 0) {
      min = v || stat.vector.new(numCols);
      f = numRows % 2;
      for (i = 0; i < numCols; i++) {
        min[i] = m[0][i];
        for (j = 0; j < numRows - f; j += 2) {
          min0 = m[j][i] > m[j + 1][i] ? m[j + 1][i] : m[j][i];
          min[i] = min[i] > min0 ? min0 : min[i];
        }
        if (f) {
          min0 = m[numRows - 1][i];
          min[i] = min[i] > min0 ? min0 : min[i];
      ...
      min = v || stat.vector.new(numRows);
      f = numCols % 2;
      for (i = 0; i < numRows; i++) {
        min[i] = m[i][0];
        for (j = 0; j < numCols - f; j += 2) {
          min0 = m[i][j] > m[i][j + 1] ? m[i][j + 1] : m[i][j];
          min[i] = min[i] > min0 ? min0 : min[i];
        }
        if (f) {
          min0 = m[i][numCols - 1];
  ...
  stat.matrix.std = function(m, dim, v) {
    var numRows = m.length, numCols = m[0].length, i, j, std, s, variance, mean, meansqr;
    dim = dim || 0;
    if (dim === 0) {
      std = v || stat.vector.new(numCols);
      mean = stat.matrix.mean(m, 0);
      for (i = 0; i < numCols; i++) {
        s = 0;
        meansqr = mean[i] * mean[i];
        for (j = 0; j < numRows; j++) s += m[j][i] * (m[j][i] - 2 * mean[i]);
        variance = (s + numRows * meansqr) / (numRows - 1);
        std[i] = Math.sqrt(variance);
      }
    } else if (dim === 1) {
      std = v || stat.vector.new(numRows);
      mean = stat.matrix.mean(m, 1);
      for (i = 0; i < numRows; i++) {
        s = 0;
        meansqr = mean[i] * mean[i];
        for (j = 0; j < numCols; j++) s += m[i][j] * (m[i][j] - 2 * mean[i]);
        variance = (s + numCols * meansqr) / (numCols - 1);
        std[i] = Math.sqrt(variance);
      }
    }
    return std;
  };
  stat.matrix.mean = function(m, dim, v) {
    var numRows = m.length, numCols = m[0].length, i, j, mean;
    dim = dim || 0;
    if (dim === 0) {
      mean = v || stat.vector.new(numCols);
      for (i = 0; i < numCols; i++) {
        mean[i] = 0;
        for (j = 0; j < numRows; j++) {
          mean[i] += (m[j][i] - mean[i]) / (j + 1);
        }
      }
    } else if (dim === 1) {
      mean = v || stat.vector.new(numRows);
      for (i = 0; i < numRows; i++) {
        mean[i] = 0;
        for (j = 0; j < numCols; j++) {
          mean[i] += (m[i][j] - mean[i]) / (j + 1);
        }
      }
    }
    return mean;
  };
  stat.matrix.row = function(m, i) {
    return m[i].slice(0);
  };
  stat.matrix.rowsByReference = function(m, a) {
    a = a || stat.array_.range(m.length);
    var numRows = a.length, rows = stat.vector.new(numRows), i;
    for (i = 0; i < numRows; i++) rows[i] = m[a[i]];
    return rows;
  };
  stat.matrix.rows = function(m, a) {
    a = a || stat.array_.range(m.length);
    var numCols = m[0].length, numRows = a.length, rows = stat.matrix.new(numRows, numCols), i, j;
    for (i = 0; i < numRows; i++) for (j = 0; j < numCols; j++) rows[i][j] = m[a[i]][j];
    return rows;
  };
  stat.matrix.col = function(m, j) {
    var numRows = m.length, col = stat.vector.new(numRows), i;
    for (i = 0; i < numRows; col[i] = m[i++][j]) ;
    return col;
  };
  stat.matrix.cols = function(m, a) {
    a = a || stat.array_.range(m[0].length);
    var numCols = a.length, numRows = m.length, cols = stat.matrix.new(numRows, numCols), i, j;
    for (i = 0; i < numRows; i++) for (j = 0; j < numCols; j++) cols[i][j] = m[i][a[j]];
    return cols;
  };
  stat.matrix.extrema = function(m, dim, vmin, vmax) {};
  stat.matrix.add = function(m0, m1) {};
  stat.matrix.scalar = function(s, m0) {
    var numRows = m0.length, numCols = m0[0].length, m = stat.matrix.copy(m0), i, j;
    for (i = 0; i < numRows; i++) for (j = 0; j < numCols; j++) m[i][j] = s * m0[i][j];
    return m;
  };
  stat.matrix.subtract = function(m0, m1) {};
  stat.matrix.toNum = function(m) {
    var numRows = m.length, i;
    for (i = 0; i < numRows; stat.array_.toNum(m[i++])) ;
    return m;
  };
  stat.matrix.euclidean = function(m0, m1) {
    var numRows0 = m0.length, numCols0 = m0[0].length, numRows1 = m1.length, numCols1 = m1[0].length, m = stat.matrix.new(numRows0, numRows1), i, j;
    if (!(numCols0 === numCols1)) return console.warn("Matrices should have the same column size");
    for (i = 0; i < numRows0; i++) for (j = 0; j < numRows1; j++) m[i][j] = stat.vector.euclidean(m0[i], m1[j]);
    return m;
  };
  if (typeof define === "function" && define.amd) define(stat); else if (typeof module === "object" && module.exports) module.exports = stat; else this.stat = stat;
}();
});
...
function rp_updateRecurrenceImg (r, buf){

    //symmetric matrix
    var m  = Math.sqrt(r.length),
        img = new Uint32Array(buf), // 4-byte mask
        i, j, ij, ji, v;
...
function rp_recurrenceImg (r) {
    ...
    var m   = r.length,
        ...
        img8 = new Uint8ClampedArray(buf),
        img32 = rp_updateRecurrenceImg(r, buf);

    //returns 1-byte view of the image
    return {img8:img8, img32:img32};
...
//self similarity
//
//distf - metric function 
function rp_updateDistanceMatrix(d, x, distf){
...
//
//distf - metric function 
function rp_distanceMatrix (x, distf){

    var m = x.length,
        d = stat.float32Array(m*m, 0);


    return rp_updateDistanceMatrix(d, x, distf);
...
//returns 1-byte view (Uint8ClampedArray) image
rp.img = function ( r ){
...
rp.imgFlipVertically = function (img){

    var flipped = new Uint32Array(img.buffer), // 4-byte mask
        n = Math.sqrt( flipped.length ), //rp image (square) width (= height)
        m = ~~( n / 2),
        i, j, ij, ij_f, v;
...
rp.imgFlipHorizontally = function (img){

    var flipped = new Uint32Array(img.buffer), // 4-byte mask
        m = Math.sqrt( flipped.length ), //rp image (square) width (= height)
        n = ~~( m / 2),
        i, j, ij, ij_f, v;
...
rp.updateDistanceMatrix = rp_updateDistanceMatrix;

rp.updateRecurrenceMatrix = rp_updateRecurrenceMatrix;

//
// Legacy --- will be removed in the next version  
//
rp.toimg = function(data, img, w, h, eps, epsnet){
...
//
rp.sym = function (x, distfname, eps){

    var distf = metric[distfname] || metric.l2,
        d = rp_distanceMatrix (x, distf),
...
var rplot = function(){
    ...
    function initData(d) {
        ...
        rpDistMatrix = new Float32Array(s);
        canvasOffScreen.setAttribute('width', imgWidth);
        canvasOffScreen.setAttribute('height', imgHeight);
        ...
        //TODO: assumes self-similarity RP
        ...
        if (s) rpimage = ctxOffScreen.getImageData(0, 0, imgWidth,imgHeight);

        initScale();

    }

    function _rplot(node, d){


        root = node instanceof selection ? node.node() : node;
    ...
    function handleBrush(){
        ...
        console.log('rp brushing...', s);

    }

    function addBrush() {

        brushInteractor = d3brush()
    ...
    function updateRP(){
        ...
        const sym = rp.sym(data.x, distfname, eps);

        rpDistMatrix = sym.d;
        rpBinary = sym.r;

        const rpImg  = rp.img( rpBinary );
        data32 = rpImg.img32;
        buf8 = rpImg.img8;
    ...
    function updateRPImg(){
        ...
        rpBinary = rp.updateRecurrenceMatrix(rpBinary, rpDistMatrix, eps);
    ...
    function update(){
        ...
        if ( rpdataDirty ) updateRP();
        else if( epsDirty ) updateRPImg();

        if (offScreenDirty) renderOffScreen();
    ...
    _rplot.rqa = function(){
        ...
        // if(rpdata) return( rqa(rpdata, data.x.length,  eps) );

        if(rpBinary) return rqa(rpBinary);

    };


    _rplot.epsnet = function(){
    ...
    _rplot.patternHighlight = function(p){
    ...
    _rplot.resetHighlight = function(){

        offScreenDirty = true;

        initArray(activeDomain,0);

        // rp.toimg(rpdata, data32, imgWidth, imgHeight, eps, epsnet);

        data32 = rp.img(rpBinary);
    ...
    _rplot.boxHighlight = function(e){
    ...
    _rplot.scale = function(_){

        if (!arguments.length) return {xs:xScale, ys:yScale};
    ...
    _rplot.distanceMatrix = function(){
        return rpDistMatrix;
    };

    _rplot.data = function(_){
    ...
    _rplot.registerListener = function(layer, type, fn, context) {
