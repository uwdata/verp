...
angular.module('verpApp')
    .controller('SceneCtrl', ['$scope', 'EventService', 'GazeAnalytics',
        function ($scope, EventService, GazeAnalytics) {
            ...
            $scope.show = {
                ...
                saccadePath:false,
                heatmap:false,
                fixationHeatmap:false,
                saccadeHeatmap:false,
                focusmap:false,
                fixationFocusmap:false,
            ...
            $scope.saccadeVisibility = function(){
                ...
                if($scope.show.saccades){

                    for(; i < n; i++)
                        $scope.visibility[i] = ($scope.event[i] === 1) ? 1 :
                            ($scope.show.fixations) ? 1 : 0;

                } else if($scope.show.fixations){
            ...
            $scope.fixationVisibility = function(){
                ...
                if($scope.show.fixations){
                    for(; i < n; i++)
                        $scope.visibility[i] = ($scope.event[i] === 0) ? 1 : ($scope.show.fixations) ? 1 : 0;

                }else if($scope.show.saccades) {
                    for (; i < n; i++)
                        $scope.visibility[i] = ($scope.event[i] === 0) ? 0 : ($scope.show.saccades) ? 1 : 0;
            ...
            function classifyIDT(){
                ...
                $scope.scanPath = GazeAnalytics.classifyIDT($scope.points, $scope.gvec, $scope.dispersion.threshold, $scope.time);
                ...
                if($scope.show.saccades) $scope.saccadeVisibility();
                if($scope.show.saccades) $scope.fixationVisibility();
            ...
            function classifyIVT(){
                ...
                if($scope.show.saccades)  $scope.saccadeVisibility();
                if($scope.show.fixations) $scope.fixationVisibility();
            ...
            function init(e,  d) {
                ...
                $scope.duration = ts * ( $scope.time[$scope.time.length-1] - $scope.time[0] );
