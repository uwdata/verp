!function() {
  ...
  chopper.parseIDF = function(text) {
    var h = chopper_getHeader(text), t = chopper_getTrials(h.rest), header = chopper_trim(h.header);
    header.push("## img:	");
  ...
  function chopper_getTrials(lines) {
    var n = lines.length, t = [], m = [], j = -1, i, indx, tokens, currdata;
    for (i = 0; i < n; i++) {
